...
class Dinov3Backbone(nn.Module):
    ...
    def forward_with_intermediates(self, x, extra_embed=None):
        """
        Encode a RGB image and return both final output and intermediate layer features.
        Args:
            - x: torch.Tensor of shape [bs,3,w,h]
            - extra_embed: Optional extra embeddings (not implemented)
        Return:
            - y: torch.Tensor of shape [bs,C,H,W] - final layer output
            - intermediate_features: list of torch.Tensor - features from each block
        """
        assert extra_embed is None, "Not Implemented Yet"

        # Get intermediate features from all blocks
        n_blocks = self.encoder.n_blocks
        intermediate_outputs = self.encoder.get_intermediate_layers(
            x, n=n_blocks, reshape=True, norm=True
        )

        # Final output is the last intermediate
        y = intermediate_outputs[-1]

        # Convert list to proper format for avatar decoder
        # Each intermediate has shape [B, C, H, W]
        intermediate_features = list(intermediate_outputs)
