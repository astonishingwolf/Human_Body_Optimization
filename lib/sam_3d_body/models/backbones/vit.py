...
class ViT(nn.Module):
    ...
    def forward_features(self, x, extra_embed=None):
        ...
        if self.pos_embed is not None:
            # Resize pos_embed if spatial resolution changes (e.g., cropping)
            ...
            if pos_embed.shape[1] != x.shape[1] + 1:
                pos_embed = get_abs_pos(
                    ...
                    Hp,
                    Wp,
                    self.patch_embed.origin_patch_shape[0],
                    self.patch_embed.origin_patch_shape[1],
                    has_cls_token=True,
                )
    ...
    def forward_with_intermediates(self, x, extra_embed=None):
        """
        Forward pass that also returns per-block token features (before the final norm).

        Args:
            x: Input images [B, C, H, W]
            extra_embed: Optional extra embeddings (e.g., ray conditioning) broadcast to tokens

        Returns:
            final_embeddings: [B, C, H', W'] spatial feature map after last_norm
            intermediate_features: list of length depth with token features [B, N_tokens, C]
        """
        B, C, H, W = x.shape
        x, (Hp, Wp) = self.patch_embed(x)
        ...
        for blk in self.blocks:
            # Capture pre-attention normed tokens for cross-attn consumers
            pre_attn = blk.norm1(x)
            if self.use_checkpoint:
                x = checkpoint.checkpoint(blk, x)
            else:
                x = blk(x)
            intermediate_features.append(pre_attn)

        x = self.last_norm(x)
        xp = x.permute(0, 2, 1).reshape(B, -1, Hp, Wp).contiguous()

        return xp, intermediate_features
