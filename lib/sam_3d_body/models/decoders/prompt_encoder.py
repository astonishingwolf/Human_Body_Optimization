...
class PromptEncoder(nn.Module):
    ...
    def get_mask_embeddings(
        ...
        if masks is not None:
            # Cast mask to match mask_downscaling weight dtype to avoid type mismatch
            # when running with mixed precision (bf16/fp16)
            weight_dtype = self.mask_downscaling[0].weight.dtype
            mask_embeddings = self.mask_downscaling(masks.to(weight_dtype))
        else:
