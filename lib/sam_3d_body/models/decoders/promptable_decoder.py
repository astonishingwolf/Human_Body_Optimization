...
class PromptableDecoder(nn.Module):
    ...
    def forward(
        ...
        # Collect intermediate features if requested
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            # Collect intermediate features (token + image embeddings after each layer)
            if get_intermediate_features:
                intermediate_features.append(
                    {
                        "token_embeddings": token_embedding.clone(),
                        "image_embeddings": image_embedding.clone(),
                    }
                )
        ...
        if self.do_interm_preds:
            ...
            if get_intermediate_features:
                return out, all_pose_outputs, intermediate_features
