...
class MHRHead(nn.Module):
    ...
    def forward(
        ...
        """
        # MHR sparse operations don't support half precision (fp16/bf16)
        # Store original dtype and convert to fp32 for all operations
        original_dtype = x.dtype

        # Disable autocast and convert inputs to fp32
        with torch.amp.autocast(device_type='cuda', enabled=False):
            x = x.float()
            if init_estimate is not None:
                init_estimate = init_estimate.float()
            ...
            # Convert tensor outputs back to original dtype
            for key, value in output.items():
                if isinstance(value, torch.Tensor) and value.dtype == torch.float32:
                    output[key] = value.to(original_dtype)
